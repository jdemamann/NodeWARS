...
function drawTaperedPath(ctx, sx, sy, cpx, cpy, ex, ey, t0, t1, widthBase, widthTip, segmentCount) {
...
export class TentRenderer {
  static draw(ctx, t) {
    ...
    if (!atk || isRev) {
      ...
      drawTaperedPath(ctx, esx, esy, cp.x, cp.y, etx, ety, sT, visEnd, wBase, wTip, segmentCount);
      ctx.fillStyle   = col;
      ctx.globalAlpha = alpha;
      ctx.fill();
    }
